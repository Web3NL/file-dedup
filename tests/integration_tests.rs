...
fn create_integration_test_structure() -> TempDir {
    ...
    create_test_file(base_path.join("pictures").as_path(), "photo.jpg", photo_content);
    create_test_file(base_path.join("downloads").as_path(), "photo_copy.jpg", photo_content);
    create_test_file(base_path.join("documents").as_path(), "backup_photo.jpg", photo_content);

    // Create document duplicates
    let doc_content = b"This is a test document with some content for duplicate detection testing.";
    create_test_file(base_path.join("documents").as_path(), "report.txt", doc_content);
    create_test_file(base_path.join("downloads").as_path(), "report_v2.txt", doc_content);

    // Create unique files
    create_test_file(base_path.join("documents").as_path(), "unique_doc.txt", b"Unique document content");
    create_test_file(base_path.join("pictures").as_path(), "unique_image.png", b"UNIQUE_PNG_DATA");
    create_test_file(base_path.join("nested/deep/folder").as_path(), "deep_file.txt", b"Deep nested file");

    // Create files with same size but different content (hash collision test)
    create_test_file(base_path.join("documents").as_path(), "same_size_1.txt", b"AAAA");
    create_test_file(base_path.join("downloads").as_path(), "same_size_2.txt", b"BBBB");
    ...
    let large_content = b"Large file content for testing performance with bigger files. ".repeat(100);
    create_test_file(base_path.join("documents").as_path(), "large1.dat", &large_content);
    create_test_file(base_path.join("downloads").as_path(), "large2.dat", &large_content);
...
fn test_end_to_end_duplicate_detection() {
    ...
    let groups_by_count: Vec<_> = duplicate_groups.iter()
        .map(|g| (g.files.len(), g.size))
        .collect();
    
    // Should have one group with 3 files and two groups with 2 files each
    assert!(groups_by_count.iter().any(|(count, _)| *count == 3), "Should have one group with 3 duplicates");
    assert_eq!(groups_by_count.iter().filter(|(count, _)| *count == 2).count(), 2, "Should have two groups with 2 duplicates each");
}
...
fn test_cli_verbose_mode() {
    ...
    assert!(output.status.success(), "CLI with verbose should run successfully");
...
fn test_cli_no_duplicates_found() {
    ...
    assert!(output.status.success(), "CLI should run successfully on unique files");
